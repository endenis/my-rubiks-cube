...
var Cube = function(i, j, k) {
    ...
    this.normal_material = new THREE.MeshFaceMaterial(materials);
    this.mesh = new THREE.Mesh(this.geometry, this.normal_material);
    this.mesh.position = new THREE.Vector3(size * (i - 1), size * (j - 1), size * (k - 1));
}

var highlighted_material = new THREE.MeshLambertMaterial({ color: 0xFFCCCC });
...
var next_cube_index = function(index) {
...
function init() {
    ...
    document.addEventListener('mouseup', function(event) {
        ...
        if (no_rotation) {
            var c = getCube(last_clicked);
            c.material = c.normal_material;
            no_rotation = false;
        }
    }, false);
...
function onDocumentMouseMove(event) {
    ...
    intersection(function(intersect) {
        ...
        if (!isMouseDown) {
            var another_cube_hover = getCube(positionToLocation(intersect.object.position));
            if (cube_hover != null && cube_hover != another_cube_hover) {
                cube_hover.mesh.material = cube_hover.normal_material;
            }
            cube_hover = another_cube_hover;
            cube_hover.mesh.material = highlighted_material;
        }
